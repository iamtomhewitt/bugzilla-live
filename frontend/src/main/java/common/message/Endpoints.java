...
public class Endpoints 
{
	...
	public static final String LIST_CONTENTS(String listName) {
		return String.format(localhost + port + "/list/%s/contents", listName);
	}

	public static final String LIST_MODIFY(String filename, String add, String remove) {
		return String.format(localhost + port + "/list/modify?name=%s&add=%s&remove=%s", filename, add, remove);
	}
	
	public static final String LIST_ADD(String filename, String contents) {
		return String.format(localhost + port + "/list/add?name=%s&contents=%s", filename, contents);
	}
	
	public static final String LIST_DELETE(String filename) {
		return String.format(localhost + port + "/list/delete?name=%s", filename);
	}
